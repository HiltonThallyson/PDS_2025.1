...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<String> handleGoogleBooksApiException(ApiException e){
        return ResponseEntity.status(HttpStatus.BAD_GATEWAY)
                .body("Não foi possível acessar a API. Tente novamente mais tarde.");
    }

    @ExceptionHandler(FavoritoException.class)
    public ResponseEntity<String> handleFavoritoException (FavoritoException e){
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
    }
    ...
    public ResponseEntity<String> handleConnectTimeOutException(ConnectTimeOutException e) {
        return ResponseEntity.status(HttpStatus.REQUEST_TIMEOUT).body("Tempo limite de conexão excedido. Por favor, tente novamente mais tarde.");
    }
